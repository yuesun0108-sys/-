...
class LeadFinder:
    ...
    def search_duckduckgo(self, query: str, limit: int = 20) -> List[str]:
        ...
        try:
            self.random_delay()
            q = query.replace(" ", "+")
            html_text = self.fetch(f"https://duckduckgo.com/html/?q={q}") or ""
    ...
    def fetch(self, url: str) -> Optional[str]:
        try:
            ...
            lt = text.lower()
            if "captcha" in lt or ("cloudflare" in lt and "attention required" in lt):
                return None
            return text
        except Exception:
            return None

    def find_country_hint(self, text: str) -> str:
        txt = text[:3000]
    ...
    def parse_brand_name(self, home_html: str, domain: str) -> str:
        m = TITLE_RE.search(home_html or "")
    ...
    def extract_footer_links(self, home_html: str, base: str) -> List[str]:
        foot = home_html.lower().split("</footer>")
        if not foot:
            return []
        last_footer = foot[0][-8000:]
        return [urljoin(base, u) for u in HREF_RE.findall(last_footer)]
    ...
    def crawl_site(self, website: str) -> List[Lead]:
        ...
        for page_url in to_visit:
            ...
            for em in emails:
                ...
                if et == "low_value":
                    continue
                brand = self.parse_brand_name(home_html, domain)
                ...
                leads.append(Lead(
                    ...
                    country_or_region=self.find_country_hint(site_text),
...
def save_outputs(leads: List[Lead], out_dir: Path):
    ...
    headers = [
        "Brand Name", "Website", "Country or Region", "Product Category", "Main Products",
        "Email", "Email Type", "Contact Page URL", "Evidence URL", "Frame Material",
        "DTC Fit Score 1 to 5", "Potential Customer Score 1 to 5", "Reason", "Notes"
    ]
    ...
    try:
        ...
        # Keep CSV as guaranteed output even if openpyxl is unavailable
        pass

    # Email-only outputs for users who only need contacts.
    email_rows: List[Dict[str, str]] = []
    email_seen = set()
    for row in rows:
        email = row["Email"].strip().lower()
        if not email or email in email_seen:
            continue
        ...
        email_rows.append(
            {
                ...
                "Website": row["Website"],
                "Contact Page URL": row["Contact Page URL"],
            }
        )
    email_csv = out_dir / "emails_only.csv"
    with email_csv.open("w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(
            f, fieldnames=["Email", "Email Type", "Brand Name", "Website", "Contact Page URL"]
        )
        writer.writeheader()
        writer.writerows(email_rows)
    (out_dir / "emails_only.txt").write_text(
        "\n".join(r["Email"] for r in email_rows) + ("\n" if email_rows else ""),
        encoding="utf-8",
    )

    email_txt = out_dir / "emails_only.txt"
    return csv_path, xlsx_path, email_csv, email_txt, len(rows), len(email_rows)
...
def main():
    ...
    csv_path, xlsx_path, email_csv, email_txt, count, email_count = save_outputs(all_leads, Path(args.output_dir))
    print(f"Saved {count} leads to {csv_path} and {xlsx_path}")
    print(f"Saved {email_count} unique emails to {email_csv} and {email_txt}")
    if count < 50:
